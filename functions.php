<?php
function zeintyres_theme_setup() {
    add_theme_support('title-tag');
    add_theme_support('post-thumbnails');
    add_theme_support('html5', array('search-form', 'comment-form', 'gallery', 'caption', 'style', 'script'));
    add_theme_support('custom-logo');
    register_nav_menus(array(
        'primary' => __('Primary Menu', 'zeintyres'),
    ));
}
add_action('after_setup_theme', 'zeintyres_theme_setup');

function zeintyres_get_contact_field($key, $post_id = null, $default = '') {
    $post_ids = array();
...
function zeintyres_customize_register($wp_customize) {
    $wp_customize->add_section('zeintyres_contact_section', array(
        'title' => __('Contact', 'zeintyres'),
        'priority' => 30,
    ));
    $wp_customize->add_setting('zeintyres_phone_number', array(
        'default' => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    $wp_customize->add_control('zeintyres_phone_number', array(
        'label' => __('Phone Number', 'zeintyres'),
        'section' => 'zeintyres_contact_section',
        'type' => 'tel',
    ));
    $wp_customize->add_setting('zeintyres_service_area', array(
        'default' => 'Melbourne Wide - Specializing in Northern Suburbs',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    $wp_customize->add_control('zeintyres_service_area', array(
        'label' => __('Service Area', 'zeintyres'),
        'section' => 'zeintyres_contact_section',
        'type' => 'text',
    ));
    $wp_customize->add_setting('zeintyres_open_hours', array(
        'default' => 'Mon-Fri: 9:00am - 5:30pm | Sat: 9:00am - 3:00pm',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    $wp_customize->add_control('zeintyres_open_hours', array(
        'label' => __('Open Hours', 'zeintyres'),
        'section' => 'zeintyres_contact_section',
        'type' => 'text',
    ));
}
add_action('customize_register', 'zeintyres_customize_register');

function zeintyres_enqueue_assets() {
    wp_enqueue_style('zeintyres-google-fonts', 'https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700;800&family=Work+Sans:wght@400;500;600&family=JetBrains+Mono:wght@700&display=swap', array(), null);
    wp_enqueue_style('zeintyres-material-icons', 'https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:wght,FILL@100..700,0..1&display=swap', array(), null);
    wp_enqueue_style('zeintyres-theme-style', get_stylesheet_directory_uri() . '/assets/css/theme.css', array(), filemtime(get_stylesheet_directory() . '/assets/css/theme.css'));

    wp_register_script('zeintyres-tailwind', 'https://cdn.tailwindcss.com?plugins=forms,container-queries', array(), null, false);
    wp_enqueue_script('zeintyres-tailwind');
    ...
    wp_add_inline_script('zeintyres-tailwind', $tailwind_config, 'after');
}
add_action('wp_enqueue_scripts', 'zeintyres_enqueue_assets');

function zeintyres_nav_menu_item_classes($classes, $item, $args, $depth) {
    if ('primary' === $args->theme_location) {
        $classes[] = 'inline-block';
    }
    return $classes;
}
add_filter('nav_menu_css_class', 'zeintyres_nav_menu_item_classes', 10, 4);

class Zeintyres_Primary_Walker extends Walker_Nav_Menu {
...
function zeintyres_menu_fallback() {
